...
class SalesPrint extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: "Proforma Report".toLabel()),
      body: PdfPreview(
      ...
      pw.MultiPage(
        ...
        build: (context) => [
              ...
              data: <List<String>>[
                ...
                ...attendanceList.map(
                  (data) => [
                    ...
                    double.parse(data.payable) > double.parse(data.payment)
                        ? 'Unpaid'
                        : 'Paid',
                    data.date,
                  ],
          ...
          pw.Row(
            ...
            children: [
              pw.Spacer(),
              pw.Text("Total Amount Unpaid : "),
              pw.Text(Utils()
                  .formatPrice((attendanceList
                          .map((e) => double.parse(e.payable))
                          .reduce((value, element) => value + element)) -
                      (attendanceList
                          .map((e) => double.parse(e.payment))
                          .reduce((value, element) => value + element)))
                  .toString())
            ],
          ),
